...
{
using WPMLData = std::pair<wcs::KMLDocument, wcs::WPMLDocument>;
...
inline std::optional<WPMLData> LIB_API parseKMZOfDJI(const std::string& kmzPath);
inline bool LIB_API createKMZOfDJI(const WPMLData& data, const std::string& kmzPath);
}; // namespace wpml_codec::core
